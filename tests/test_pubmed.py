...
def test_get_pubmed_metadata_pmid_empty():
    df = get_pubmed_metadata_pmid([])
    assert isinstance(df, pd.DataFrame)
    assert df.empty

@respx.mock
def test_get_pubmed_metadata_pmid_success(mock_xml_response):
...
@respx.mock
def test_get_pubmed_metadata_pmid_network_error():
    respx.post("https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi").mock(
...
@respx.mock
def test_get_pubmed_metadata_pmid_runtime_error_event_loop(mock_xml_response):
